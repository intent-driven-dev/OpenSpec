...
- [ ] 1.1 Add an optional `format` block to `SchemaYamlSchema` in `src/core/artifact-graph/types.ts` (extension, requirement, scenario, and delta markers; section-style keys OR inline `marker`/`rename`).
...
- [ ] 4.1 Golden regression: existing Markdown spec fixtures parse/merge/archive byte-identically (proves zero migration).
...
- [ ] 5.1 Document the schema `format` block (fields, both delta dialects, scenario-rigor behavior) and a worked non-Markdown example.
