 ...
 * full.
 */
export function extractRequirementBody(bodyLines: string[], boundaryRegex: RegExp = HEADER_LINE): string {
  ...
  for (let i = 0; i < bodyLines.length; i++) {
    ...
    if (boundaryRegex.test(line)) break; // first scenario or stray divider
 ...
 */
export function countScenarios(bodyLines: string[], scenarioRegex: RegExp = SCENARIO_HEADER): number {
  const mask = buildCodeFenceMask(bodyLines);
  let count = 0;
  for (let i = 0; i < bodyLines.length; i++) {
    if (mask[i]) continue;
    if (scenarioRegex.test(bodyLines[i])) count++;
  }
  return count;
}
